(** Input syntax (no mixfix parsing yet) *)
...
type expr = basic list (* must be non-empty *)

(** exoressions that are parsed by Menhir *)
and basic =
  | Expr of expr         (** this arises when a basic expression is inside parentheses *)
  | Builtin of string    (** a builtin value *)
...
type toplevel_cmd =
  ...
  | Mixfix of string * int * assoc (** declaration of a new operator *)
